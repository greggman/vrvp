...
Properties {
    ...
    [KeywordEnum(6 Frames Layout, Latitude Longitude Layout, FishEye Layout, Flat)] _Mapping("Mapping", Float) = 1
    [Enum(360 Degrees, 0, 180 Degrees, 1, FullFrame, 2)] _ImageType("Image Type", Float) = 0
...
SubShader {
    ...
    Pass {
        ...
        inline float2 ToRadialCoords(float3 coords)
        {
            ...
            return frac(float2(0.5,1.0) - sphereCoords + float2((1 - _ImageType) * 0.25, 0));
        }
#endif

#ifdef _MAPPING_FISHEYE_LAYOUT
        inline float2 ToFisheyeCoords(float3 coords, float xScale)
        {
            float3 normalizedCoords = normalize(coords);
            float r = (_ImageType + 1.0) * atan2(length(normalizedCoords.xy), normalizedCoords.z) / UNITY_PI;
            float theta = atan2(normalizedCoords.y, normalizedCoords.x);
            float2 uv = float2(cos(theta), sin(theta)) * r * 0.5 + 0.5;
            return float2(uv.x * 0.5 * (3 - xScale), uv.y);
        }
#endif
...
#endif
#ifdef _MAPPING_FISHEYE_LAYOUT
            tc = ToFisheyeCoords(i.texcoord, i.image180ScaleAndCutoff[0]);
